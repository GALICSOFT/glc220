...
INT  LcStv_InputOpen(void* hWnd);												// Input device open. hWnd is need only in Microsoft windows.
...
INT  LcStv_InputReadMts(INT idx, LC_INPUT_TS* ts);								// gain Multi-touch Screen coordinate and state. ex) LC_INPUT_TS t; LcStv_InputReadMts([0,16], &t);
INT  LcStv_InputWriteMts(INT idx, LC_INPUT_TS* ts=NULL);						// write the ts data. NULL sets to default. ex) LC_INPUT_TS t...; LcStv_InputWriteMts([0,16], &t);
...
INT  LcStv_SensorWrite(INT n, LC_SENSOR* v);		// Write data
INT  LcStv_SensorRead(INT n, LC_SENSOR* v);			// Read data
