...
// Reference Count of instance for ILC_Base object
INT LcSys_DestroyObject(PLC_OBJECT*);									// Release the GLC Object. Same as the ILC_Object::Release().
INT LcSys_RefCountObject();												// Get the Remained GLC Object Reference Count.
INT LcSys_IncRefCountObject();											// Increase the Reference Count. when the object is created, this function must be called.
INT LcSys_DecRefCountObject();											// Decrease the Reference Count. when the object is destroy, this function must be called.

//System Initialize and Close
INT  LcSys_Version(UINT* dVer, UINT* dDate);							// SDK Version: Version(Major(Hi-word), Minor(Low-word), Build-date
...
INT LcSys_WindowSize (INT* Width, INT* Height, INT bWrite=0);			// screen width, screen height, bWrite = 0 ==> get the screen width and height. else ==> set the screen size
...
enum ELC_SYSTEM												// System command and values
{
	...
	LCSYS_WINDOW_SIZE		= 0x00010011,					// int[2], screen width, screen height. same to LcSys_WindowSize
...
typedef struct tagTLC_LIGHT									// GLC Lighting
{
...
typedef struct tagTLC_MATERIAL								// GLC Material
{
	INT			Fce;										// Face
...
PLC_DEVICE	LcDev_GetDevice  (void );									// Global PLC_DEVICE
LC_HANDLE	LcDev_GetMainApp (void );									// Main Application Pointer
LC_HANDLE	LcDev_GetSprite  (INT n);									// 2D Sprite Pointer : Max n = LC_MAX_SPRITE
LC_HANDLE	LcDev_GetInput   (void );									// Input Pointer
LC_HANDLE	LcDev_GetNetwork (void );									// Network Pointer
LC_HANDLE	LcDev_GetUserData(INT n);									// User Data Pointer 1

void		LcDev_SetMainApp (LC_HANDLE);
void		LcDev_SetSprite  (LC_HANDLE, INT n);
void		LcDev_SetInput   (LC_HANDLE);
void		LcDev_SetNetwork (LC_HANDLE);
void		LcDev_SetUserData(LC_HANDLE, INT n);
...
// ILC_Sprite draw is immediately mode.
DCL_INTERFACE ILC_Sprite : public ILC_Object							// ILC_Sprite draw is immediately mode.
{
...
INT LcDev_CreateSprite(char* sCmd
					...
					, LC_HANDLE p1 = NULL								// int pointer. Sprite type. default is LC_OBJ_SPRITE. game model needs LC_OBJ_MODEL2D
...
// ILC_Sprite draw is immediately mode. but ILC_SpriteMx draw mode is delayed draw.
...
DCL_INTERFACE ILC_SpriteMx : public ILC_Sprite
{
	DCL_CLASS_DESTROYER(	ILC_SpriteMx	);
	...
	virtual INT	Draw()=0;												// it is going to render the assembled vertex list.
	virtual INT	DrawAndFlush()=0;										// Absolutely draw and clear the assembled vertex list
	virtual	INT	Clear()=0;												// Clear the assembled vertex list
};

typedef	ILC_SpriteMx*	PLC_SPRITEMX;

// Create Instance
INT LcDev_CreateSpriteMx(char* sCmd
					   , PLC_SPRITEMX* pOut								// Output instance
...
DCL_INTERFACE ILC_AppFrame : public ILC_Object
{
	...
	virtual INT		Create(PLC_ARG =NULL)=0;									// call when the instance create time.
	...
	virtual INT		SetAttrib(UINT nCmd, LC_HANDLE pData)=0;					// Extended the Query. set the attribute value
	virtual INT		GetAttrib(UINT nCmd, LC_HANDLE pData)=0;					// Extended the Query. get the attribute value
...
enum ELC_FONT
{
	...
	LC_FONT_RECT			,								// Character Rectangle. needs LC_RECTF. if you use this data to postion you needs scale up value by LC_FONT_SCALEUP
