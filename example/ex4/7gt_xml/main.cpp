...
int main(int argc, char** argv)
{
		...
		
	{
...

		{
			...
			TiXmlDocument doc( "xml/demotest.xml" );
			doc.Parse( demoStart );
		...
		if ( !loadOkay )
		{
			printf( "Could not load test file 'xml/demotest.xml'. Error='%s'. Exiting.\n", doc.ErrorDesc() );
			exit( 1 );
		}
	...
	printf ("\n** UTF-8 **\n");
	{
		TiXmlDocument doc( "xml/utf8test.xml" );
		doc.LoadFile();
		if ( doc.Error() && doc.ErrorId() == TiXmlBase::TIXML_ERROR_OPENING_FILE ) {
			printf( "WARNING: File 'xml/utf8test.xml' not found.\n"
		...
		else
		{
			...
			doc.SaveFile( "xml/utf8testout.xml" );
			...
			FILE* saved  = fopen( "xml/utf8testout.xml", "r" );
			FILE* verify = fopen( "xml/utf8testverify.xml", "r" );
			if ( saved && verify )
			{
	...
	{
		TiXmlDocument doc( "xml/midsummerNightsDreamWithAVeryLongFilenameToConfuseTheStringHandlingRoutines.xml" );
		bool loadOkay = doc.LoadFile();
...

	{
		...
		TiXmlDocument doc( "xml/passages.xml" );
		doc.Parse( passages );
    ...
    {
		FILE* textfile = fopen( "xml/test5.xml", "w" );
		if ( textfile )
		{
			...
			TiXmlDocument doc;
            doc.LoadFile( "xml/test5.xml" );
            XmlTest( "dot in element attributes and names", doc.Error(), 0);
		}
    }

	{
		FILE* textfile = fopen( "xml/test6.xml", "w" );
		if ( textfile )
		{
            ...
            TiXmlDocument doc;
            bool result = doc.LoadFile( "xml/test6.xml" );
...

	{
		...
		TiXmlDocument doc;
		doc.Parse( doctype );
		doc.SaveFile( "xml/test7.xml" );
		doc.Clear();
		doc.LoadFile( "xml/test7.xml" );
...

	{
		...
		/* Could not repro. {
			TiXmlDocument xml;
			xml.LoadFile( "xml/EQUI_Inventory.xml" );
