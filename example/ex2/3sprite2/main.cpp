...
PLC_TEXTURE	m_vTex	= NULL;		// Texture List
...
//	if(c>20*60*20)
	...
	r	= LCXVECTOR2(0, 0);
	t	= LCXVECTOR2(100, 25);
	d	= LCXCOLOR(1,1,1,0.8F);

	r  = t + LCXVECTOR2(rc.w/2.F * s.x, rc.h/2.F * s.y);

	m_pSpt->DrawEx(m_vTex, &rc, &t, &s, &r, fRadian, &d);
...
int main(int argc, char* argv[])
{
	...
	if(LC_FAILED(LcDev_CreateTextureFromFile(NULL, &m_vTex, (LC_HANDLE)sFile)))
		goto End;
...
End:
	SAFE_RELEASE(	m_vTex		);	// Release Texture
