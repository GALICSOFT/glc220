...
int main(int argc, char* argv[])
{
...

	{
		...
		unsigned long	len = 0;
		const char*		sZipFile = (char*)MEDIA_DIR"sound/raw.zip";


		hr = Lzip_UnzipbyName(&buf, &len, sZipFile, (char*)MEDIA_DIR"groove1.gpsf", LC_TRUE);
		if(LC_FAILED(hr))
			return LC_CANNT_ALLOC;


		hr = LcxSmd_CreateALSoundFromMemory(NULL, &m_pSnd, buf, &len);
	...
	//hr = LcxSmd_CreateSoundFromFile(NULL, &m_pSnd, (char*)MEDIA_DIR"sound/groove1.wav");
